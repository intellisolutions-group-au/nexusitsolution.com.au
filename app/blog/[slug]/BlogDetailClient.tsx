        ...
        <div className="container-custom">
          <div className="grid lg:grid-cols-[1fr_380px] gap-16 relative">
            ...
            <div className="space-y-16">
              ...
              <motion.article className="w-full max-w-none">
                <ReactMarkdown
                  components={{
                    h2: ({ children }) => {
                      const id = String(children).toLowerCase().replace(/[^\w\s-]/g, "").replace(/\s+/g, "-");
                      return <h2 id={id} className="text-2xl md:text-3xl font-black text-slate-900 mt-12 mb-6 tracking-tight scroll-mt-32">{children}</h2>;
                    },
                    h3: ({ children }) => {
                      const id = String(children).toLowerCase().replace(/[^\w\s-]/g, "").replace(/\s+/g, "-");
                      return <h3 id={id} className="text-xl md:text-2xl font-bold text-slate-800 mt-8 mb-4 tracking-tight scroll-mt-32">{children}</h3>;
                    },
                    p: ({ children }) => (
                      <p className="text-slate-600 leading-relaxed mb-6 text-lg">{children}</p>
                    ),
                    ul: ({ children }) => (
                      <ul className="flex flex-col space-y-4 mb-8 mt-2">{children}</ul>
                    ),
                    ol: ({ children }) => (
                      <ol className="list-decimal list-inside space-y-4 mb-8 mt-2 text-slate-600 text-lg">{children}</ol>
                    ),
                    li: ({ children }) => (
                    ...
                    blockquote: ({ children }) => (
                      <blockquote className="border-l-4 border-primary bg-primary/5 py-4 px-6 md:px-8 rounded-r-2xl italic my-8 text-slate-700 text-lg">
                        {children}
                      </blockquote>
                    ),
                    a: ({ children, href }) => (
                      <a href={href} className="text-primary no-underline hover:underline font-medium">
                        {children}
                      </a>
                    ),
                    code: ({ children }) => (
                      <code className="text-primary bg-primary/5 px-1.5 py-0.5 rounded-md font-mono text-sm">
                        {children}
                      </code>
                    ),
                  }}
